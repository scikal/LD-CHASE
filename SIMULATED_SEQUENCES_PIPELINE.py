...
def contrast_crossovers_wrapper(disomy_obs_filename,monosomy_obs_filename,chr_id,sp,ancestral_makeup,model,min_reads,max_reads,output_dir,ref_dir):
...
def main(depth,sp,chr_id,read_length,min_reads,max_reads,work_dir,admixture_proportions=[],mismatch=False):
    ...
    fns = MixHaploids_wrapper(*sim_obs_tabs, read_length=read_length, depth=depth, scenarios=('disomy','monosomy'),
                                    ...
                                    distant_admixture=admixture_proportions if admixture_proportions!=[0.5,0.5] else [])
    disomy_obs_filename, monosomy_obs_filename = fns
    ...
    contrast_crossovers_wrapper(disomy_obs_filename,monosomy_obs_filename,chr_id,effective_SP,ancestral_makeup,None,min_reads,max_reads,work_dir,ref_path)
